...
exports.createRegion = async (req, res) => {
  ...
  try {
    if (isNaN(wardNo)) {
      return res.status(400).json({ message: 'Ward number must be a number' });
    }
...
exports.updateRegionById = async (req, res) => {
  ...
  try {
    const updatedRegion = await Region.findByIdAndUpdate(
      id,
      { state, district, zone, taluk, wardNo, pincode },
      { new: true, runValidators: true }
    );
...
exports.downloadVotersList = async (req, res) => {
  ...
  try {
    ...
    if (region) {
      const filePath = path.join(__dirname, 'voters-list.txt');
      const fileContent = region.voters.map(voter =>
        `Voter ID: ${voter._id}, Name: ${voter.label}, Mobile Number: ${voter.mobile_number}`
      ).join('\n');
...
exports.downloadPartiesList = async (req, res) => {
  ...
  try {
    ...
    if (region) {
      const filePath = path.join(__dirname, 'parties-list.txt');
      const fileContent = region.parties.map(party =>
        `Party ID: ${party._id}, Party Name: ${party.partyName}, Party Leader: ${party.partyLeader}, Party Symbol: ${party.partySymbol}`
      ).join('\n');
